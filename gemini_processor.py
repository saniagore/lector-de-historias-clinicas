...
def process_images_in_folder(folder_path, api_key):
    ...
    for filename in files:
        ...
        print(f"\nProcesando con Gemini: {filename}...")
        
        try:
            # Upload file to Gemini API temporarily
            print("Subiendo archivo a Google Generative AI...")
            ...
            if uploaded_file.state.name == "FAILED":
                raise Exception("El archivo falló al procesarse en Google Generative AI.")
                
            print("\nArchivo procesado. Extrayendo datos...")
            
            prompt = """
            Eres un asistente especializado en revisar historias clínicas.
            Extrae de este documento la información más importante del paciente y el texto completo reconocido.
            Debes devolver ÚNICAMENTE un objeto JSON válido con la siguiente estructura (reemplaza los valores con la información extraída):
            {
                "texto_completo": "Todo el texto puro que logres leer u OCR de la historia clínica. No resumas, devuelve el texto plano tal cual está en el documento.",
                "datos": {
                    ...
                    "fecha_ingreso": "Fecha de ingreso encontrada en el documento",
                    ...
                    "enfermedad_actual": "Descripción detallada de la enfermedad actual, evolución, síntomas y estado del paciente.",
                    "revision_sistemas": [
                        {
                    ...
                    "analisis_medico": "Análisis clínico, razonamiento médico o evaluación cualitativa que hace el profesional.",
                    "paraclinicos": [
                        {
                            "fecha": "Fecha del examen (Ej: 03/03/2026). Si no hay, pon 'No registrada'.",
                    ...
                    "diagnostico": "Impresión diagnóstica o diagnósticos principales mencionados.",
            ...
            # Delete the file from Gemini to save the user's quota/storage
            genai.delete_file(uploaded_file.name)
            print("Datos extraídos correctamente.")
            
        except Exception as e:
            print(f"Error procesando {filename} con Gemini: {str(e)}")
